...
timetemp=datetime.datetime.now().strftime("SAC_5_%Y_%m_%d_%H_%M_%S")
...
model=SAC.load('../../Data3/SAC_6',env)
...
for i in range(500):
    ...
    # obs,reward,dones,info=env.step([action[0][0],0])
    obs,reward,dones,info=env.step([0,-0.1])
    ...
    # ve.append(obs[0, 2] * 30)
    print("step",i)
    print(env.road.vehicles[0].lane_index)
    print('obs', obs)
    print('reward', reward)

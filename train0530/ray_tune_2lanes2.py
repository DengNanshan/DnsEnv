...
config={
    ...
    "gamma" :tune.grid_search([5e-1, 7e-1, 9e-1, 95e-2, 97e-2])
}
...
def training_function(config):
    ...
    for step in range(4):
        ...
        model = DQN(MlpPolicy, env, verbose=1,
                    tensorboard_log=dir + 'tensorboard_log/'+str(config["lr"])+' '+str(config["gamma"])+'/',
        ...
        model.save(dir +timetemp+' '+str(config["lr"])+' '+str(config["gamma"]))

...
config={
    ...
    "init_speed": tune.grid_search([[0.3,0.4],[0.4,0.8],[0.3,0.7],[0.7,0.8]]),
...
def training_function(config):
    ...
    for step in range(1):
        ...
        checkpoint_callback = CheckpointCallback(save_freq=1000,
                                                 ...
                                                 verbose=1)
        E = EvalCallback(eval_env=env, eval_freq=1000, log_path=dir + str(config["init_speed"])+str(config["exploration_fraction"])+'/',
                         best_model_save_path=dir + str(config["init_speed"])+str(config["exploration_fraction"])+'/')
        callbacks = CallbackList([checkpoint_callback, E])
        model.learn(25000,callback=callbacks)
        model.save(dir + str(config["init_speed"]) + str(config["exploration_fraction"])+ timetemp)
